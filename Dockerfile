...
RUN mkdir $NUTZWEB_HOME $NUTZWEB_HOME/libs $NUTZWEB_HOME/rs $NUTZWEB_HOME/ROOT $NUTZWEB_HOME/conf $NUTZWEB_HOME/props
...
RUN cd $NUTZWEB_HOME && wget https://github.com/nutzam/nutz/archive/master.zip && unzip master.zip && rm -fr master.zip && \
	cd $NUTZWEB_HOME/nutz-master && mvn -Dmaven.test.skip=true clean package install dependency:copy-dependencies && cp target/dependency/* $NUTZWEB_HOME/libs/ && \
...
RUN cd $NUTZWEB_HOME && wget https://github.com/nutzam/nutz-web/archive/master.zip && unzip master.zip &&\
	...
	cd $NUTZWEB_HOME/nutz-web-master && mvn -Dmaven.test.skip=true clean package install dependency:copy-dependencies && cp target/dependency/* $NUTZWEB_HOME/libs/ && \
	cp $NUTZWEB_HOME/nutz-master/nutz-web-1.b.52.jar $NUTZWEB_HOME/libs/ && \
	cd $NUTZWEB_HOME && rm -fr $NUTZWEB_HOME/nutz-web-master


EXPOSE 8080
VOLUME /var/lib/nutz-web
CMD /bin/bash